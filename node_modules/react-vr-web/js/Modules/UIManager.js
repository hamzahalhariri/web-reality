...
/**
 * UIManager is a Module that associates view types with their client-side
 * implementations. It can create and manage UI elements, keeping them in sync
 * with updates from the React side.
 */
export default class UIManager extends Module {
  /**
   * Construct a UIManager with a React Native Context and an OVRUI GuiSys.
   */
  constructor(rnctx, guiSys) {
    super('UIManager');
    this._rnctx = rnctx;
    this._guiSys = guiSys;
   ...
   */
  createView(tag, type, rootTag, attr) {
   ...
   */
  setChildren(tag, childrenTags) {
    // look up the view
    const viewToManage = this._views[tag];
    // iterate through the children
    for (let i = 0; i < childrenTags.length; i++) {
      // add viewToAdd as a child of viewToManage as index i
      const viewToAdd = this._views[childrenTags[i]];
      if (viewToAdd) {
  ...
  manageChildren(tag, moveFrom, moveTo, addChildTags, addAtIndices, removeFrom) {
    const cssNodeToManage = this._views[tag];
    if (!cssNodeToManage) {
      return;
    }
    ...
    for (let i = 0; i < viewsToAdd.length; i++) {
      ...
      const cssNodeToAdd = this._views[viewAtIndex.tag];
   ...
   */
  registerViewType(name, view, viewCreator) {
    this[name] = view;
  ...
  updateView(tag, type, attr) {
    ...
    for (let a in attr) {
      ...
      if (this[type] && this[type].NativeProps[a]) {
        ...
        forceLayout |= (!STYLES_THAT_DONT_ALTER_LAYOUT[a]);
      }
    }
    // force a layout if any layout styles are applied
    forceLayout && view.makeDirty();
  ...
  createRootView(tag) {
    ...
    this._rootViews[tag] = this.createView(tag, 'RCTView', tag, null);
    this._guiSys.add(this._rootViews[tag].view);
  }
  ...
  replaceExistingNonRootView(reactTag, newReactTag) {
    const view = this._views[reactTag];
    if (!view) {
      return;
    }
    view.makeDirty();
   ...
   */
  removeSubviewsFromContainerWithID(tag) {
    const view = this._views[tag];
    if (!view) {
      return;
    }
  ...
  presentLayout(view) {
    ...
    view.presentLayout && view.presentLayout();
    view.isDirty = false;
  ...
  layout() {
    ...
    for (let root in this._rootViews) {
      ...
      const rootView = this._rootViews[root];
   ...
   */
  showGazeCursor() {
    this._guiSys.setCursorEnabled(true);
  }
   ...
   */
  hideGazeCursor() {
    this._guiSys.setCursorEnabled(false);
  }

  /**
   * Util function that given text requirements returns the height of the text
   */
  measureTextHeight(text, fontSize, width, maxLineCount, callbackID) {
   ...
   */
  setBoundingBoxVisible(tag, visible) {
    const view = this._views[tag];
   ...
   */
  measureLayout(reactTag, ancestorTag, errorCallback, successCallback) {
    let view = this._views[reactTag];
    if ( !view ) {
   ...
   * that can have offset content on the screen.
   */
  measureLayoutRelativeToParent(reactTag, errorCallback, successCallback) {
    const view = this._views[reactTag];
    if ( !view ) {
   ...
   */
  configureNextLayoutAnimation(config, finished, error) {
